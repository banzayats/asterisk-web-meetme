...
if (!isset($confno))
{
	...
	while($row = $result->fetchRow())
	{
		...
		*/
		echo "<tr><td height=\"35\">$row[1]</td><td>$row[2]</td><td>$row[3]</td><td>$row[4]</td><td><a href=\"#\" onclick=\"call_add(\x27$row[1]\x27,\x27$row[2]\x27,\x27$row[3]\x27,\x27$row[4]\x27,\x27$confno\x27,\x27$book\x27)\" type=\"button\" />[ВЫЗОВ]</a><div align=\"center\" id=\"$row[2]\"></div></td></tr>\n";
	}
}
echo "</table>\n$stran";
